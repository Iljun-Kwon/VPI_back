...
from util.train_model import ViewPredictor, Log1pTransform

sys.modules['__main__'].__dict__['Log1pTransform'] = Log1pTransform

def _featurize_for_predict(subs: float, hours: float, likes: float):
...
def predict_single(bundle_path, subs, hours, like_count) -> int:
    model, scaler_X, scaler_y = load_bundle(bundle_path)
    X = _featurize_for_predict(subs, hours, like_count)
